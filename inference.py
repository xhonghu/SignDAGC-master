...
prefix = '/new_home/PHOENIX-2014-T-release-v3/PHOENIX-2014-T/'
gloss_tokenizer = {}
gloss_tokenizer['gloss2id_file'] = f'./preprocess/{dataset}/gloss2ids.pkl'
if 'phoenix' in dataset:
    gloss_tokenizer['lower_case'] = False

model_weights = f'./{dataset}/_best_model.pt'

select_id = 99 

# Load data and apply transformation
gloss_tokenizer = GlossTokenizer_S2G(gloss_tokenizer)
inputs_list = np.load(f"./preprocess/{dataset}/dev_info.npy", allow_pickle=True).item()
name = inputs_list[select_id]['fileid']
img_folder = os.path.join(prefix, "features/fullFrame-210x260px/" + inputs_list[select_id]['folder']) if 'phoenix' in dataset else os.path.join(prefix, inputs_list[select_id]['folder'])
